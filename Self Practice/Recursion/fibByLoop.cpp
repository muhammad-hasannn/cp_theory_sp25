...
void printSeq(){
    ...
    cout << "Enter no. of terms which you want in fibonacci sequence: ";
    cin >> terms;
...
void printTerm(){
    ...
    cout << "Enter the no of term(place of term) which you want to see in fibonacci sequence: ";
    cin >> term;

    if(term == 1) cout << a << endl;
    else if(term == 2) cout << b << endl;
    else{
        for(int i = 2; i < term; i++){ // (2 is 0, at this moment)
            next = (a+b);
            a = b; 
            b = next;
        }
